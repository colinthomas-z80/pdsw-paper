...
if __name__ == "__main__":
    ...
    landmark_tar = m.declare_url("file:///scratch365/cthoma26/gekkofs_deps.tar.gz", cache="always")
    
    landmark = m.declare_untar(landmark_tar, cache="always")
    ...
    for i in range(args.task_count):
        t = vine.Task(
        command = "cat $(find landmark/ -type f) > /dev/null; sleep 1",
        inputs = { landmark : {"remote_name":"landmark"} },
        )
        task_id = m.submit(t)
