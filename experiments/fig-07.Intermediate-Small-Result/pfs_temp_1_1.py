...
if __name__ == "__main__":
    ...
    for i in range(args.task_count):
        t = vine.Task(
        command = f"cat /scratch365/cthoma26/gigfile_11 > /scratch365/cthoma26/outputs/outfile.{i}; sleep 1",
        )
        task_id = m.submit(t)
        i+=1
    ...
    for i in range(args.task_count):
        t = vine.Task(f"cat /scratch365/cthoma26/outputs/outfile.{i} > /dev/null")
        task_id = m.submit(t)
